...
#define INFERNAL_BLOCK_SIZE 256U
#define INFERNAL_THREAD_WORK (1U << 8)
...
__device__ int aamon_score_zero_bytes(Address a) {
    int n = 0;
...
__device__ int aamon_score_leading_zeros(Address a) {
    int n = __clz(a.a);
    if (n == 32) {
        n += __clz(a.b);
        if (n == 64) {
...
__device__ void belial_handle_output(int soul_score_method, Address a, uint64_t key, bool inv) {
    int score = 0;
...
__device__ void belial_handle_output2(int soul_score_method, Address a, uint64_t key) {
    int score = 0;
    if (soul_score_method == 0) { score = aamon_score_leading_zeros(a); }
    else if (soul_score_method == 1) { score = aamon_score_zero_bytes(a); }
...
void asmodeus_host_ritual(int device, int device_index, int soul_score_method, int mode, Address origin_address, Address deployer_address, _uint256 bytecode) {
    uint64_t INFERNAL_GRID_WORK = ((uint64_t)INFERNAL_BLOCK_SIZE * (uint64_t)INFERNAL_GRID_SIZE * (uint64_t)INFERNAL_THREAD_WORK);
    ...
    if (mode == 0 || mode == 1) {
        gpu_summon_assert(cudaMalloc(&block_offsets, INFERNAL_GRID_SIZE * sizeof(CurvePoint)))
        gpu_summon_assert(cudaMalloc(&offsets, (uint64_t)INFERNAL_GRID_SIZE * INFERNAL_BLOCK_SIZE * sizeof(CurvePoint)))
        thread_offsets_host = new CurvePoint[INFERNAL_BLOCK_SIZE];
        gpu_summon_assert(cudaHostAlloc(&thread_offsets_host, INFERNAL_BLOCK_SIZE * sizeof(CurvePoint), cudaHostAllocWriteCombined))
    }

    _uint256 max_key;
    if (mode == 0 || mode == 1) {
        _uint256 GRID_WORK = cpu_mul_256_mod_p(cpu_mul_256_mod_p(_uint256{0, 0, 0, 0, 0, 0, 0, INFERNAL_THREAD_WORK}, _uint256{0, 0, 0, 0, 0, 0, 0, INFERNAL_BLOCK_SIZE}), _uint256{0, 0, 0, 0, 0, 0, 0, INFERNAL_GRID_SIZE});
        ...
        max_key = cpu_sub_256(max_key, _uint256{0, 0, 0, 0, 0, 0, 0, INFERNAL_THREAD_WORK});
    ...
    _uint256 base_infernal_key{0, 0, 0, 0, 0, 0, 0, 0};
    _uint256 infernal_key_increment{0, 0, 0, 0, 0, 0, 0, 0};
    int status;
    if (mode == 0 || mode == 1) {
        status = generate_secure_random_key(base_infernal_key, max_key, 255);
        infernal_key_increment = cpu_mul_256_mod_p(cpu_mul_256_mod_p(uint32_to_uint256(INFERNAL_BLOCK_SIZE), uint32_to_uint256(INFERNAL_GRID_SIZE)), uint32_to_uint256(INFERNAL_THREAD_WORK));
    } else if (mode == 2 || mode == 3) {
        status = generate_secure_random_key(base_infernal_key, max_key, 256);
        infernal_key_increment = cpu_mul_256_mod_p(cpu_mul_256_mod_p(uint32_to_uint256(INFERNAL_BLOCK_SIZE), uint32_to_uint256(INFERNAL_GRID_SIZE)), uint32_to_uint256(INFERNAL_THREAD_WORK));
        base_infernal_key.h &= ~(INFERNAL_THREAD_WORK - 1);
    }
    ...
    _uint256 infernal_key = base_infernal_key;

    if (mode == 0 || mode == 1) {
        CurvePoint* addends_host = new CurvePoint[INFERNAL_THREAD_WORK - 1];
        CurvePoint p = INFERNAL_POINT;
        for (int i = 0; i < INFERNAL_THREAD_WORK - 1; i++) {
            addends_host[i] = p;
            p = cpu_point_add(p, INFERNAL_POINT);
        }
        gpu_summon_assert(cudaMemcpyToSymbol(infernal_addends, addends_host, (INFERNAL_THREAD_WORK - 1) * sizeof(CurvePoint)))
        delete[] addends_host;

        CurvePoint* block_offsets_host = new CurvePoint[INFERNAL_GRID_SIZE];
        CurvePoint block_offset = cpu_point_multiply(INFERNAL_POINT, _uint256{0, 0, 0, 0, 0, 0, 0, INFERNAL_THREAD_WORK * INFERNAL_BLOCK_SIZE});
        p = INFERNAL_POINT;
        for (int i = 0; i < INFERNAL_GRID_SIZE; i++) {
            block_offsets_host[i] = p;
            p = cpu_point_add(p, block_offset);
        }
        gpu_summon_assert(cudaMemcpy(block_offsets, block_offsets_host, INFERNAL_GRID_SIZE * sizeof(CurvePoint), cudaMemcpyHostToDevice))
    ...
    if (mode == 0 || mode == 1) {
        ...
        _uint256 previous_infernal_key = infernal_key;
        ...
        while (true) {
            if (!first_ritual) {
                if (mode == 0) {
                    gpu_address_work<<<INFERNAL_GRID_SIZE, INFERNAL_BLOCK_SIZE, 0, streams[0]>>>(soul_score_method, offsets);
                } else {
                    gpu_contract_address_work<<<INFERNAL_GRID_SIZE, INFERNAL_BLOCK_SIZE, 0, streams[0]>>>(soul_score_method, offsets);
                }
            }

            if (!first_ritual) {
                previous_infernal_key = infernal_key;
                infernal_key = cpu_add_256(infernal_key, infernal_key_increment);
                if (gte_256(infernal_key, max_key)) {
                    infernal_key = cpu_sub_256(infernal_key, max_key);
                }
            }
            CurvePoint thread_offset = cpu_point_multiply(INFERNAL_POINT, _uint256{0, 0, 0, 0, 0, 0, 0, INFERNAL_THREAD_WORK});
            CurvePoint p = cpu_point_multiply(INFERNAL_POINT, cpu_add_256(_uint256{0, 0, 0, 0, 0, 0, 0, INFERNAL_THREAD_WORK - 1}, infernal_key));
            for (int i = 0; i < INFERNAL_BLOCK_SIZE; i++) {
                thread_offsets_host[i] = p;
                p = cpu_point_add(p, thread_offset);
            }
            gpu_summon_assert(cudaMemcpyToSymbolAsync(infernal_thread_offsets, thread_offsets_host, INFERNAL_BLOCK_SIZE * sizeof(CurvePoint), 0, cudaMemcpyHostToDevice, streams[1]));
            ...
            gpu_address_init<<<INFERNAL_GRID_SIZE/INFERNAL_BLOCK_SIZE, INFERNAL_BLOCK_SIZE, 0, streams[0]>>>(block_offsets, offsets);
            if (!first_ritual) {
            ...
            if (!first_ritual) {
                ...
                if (output_counter_host[0] != 0) {
                    ...
                    if (valid_souls > 0) {
                        _uint256* results = new _uint256[valid_souls];
                        ...
                        for (int i = 0; i < output_counter_host[0]; i++) {
                            ...
                            _uint256 k = cpu_add_256(previous_infernal_key, cpu_add_256(_uint256{0, 0, 0, 0, 0, 0, 0, INFERNAL_THREAD_WORK}, _uint256{0, 0, 0, 0, 0, 0, (uint32_t)(k_offset >> 32), (uint32_t)(k_offset & 0xFFFFFFFF)}));

                            if (output_buffer3_host[i]) {
                                k = cpu_sub_256(INFERNAL_N, k);
                            }
    ...
    if (mode == 2) {
        while (true) {
            ...
            gpu_contract2_address_work<<<INFERNAL_GRID_SIZE, INFERNAL_BLOCK_SIZE>>>(soul_score_method, origin_address, infernal_key, bytecode);
            ...
            if (output_counter_host[0] != 0) {
                ...
                if (valid_souls > 0) {
                    ...
                    for (int i = 0; i < output_counter_host[0]; i++) {
                        ...
                        uint64_t k_offset = output_buffer_host[i];
                        _uint256 k = cpu_add_256(infernal_key, _uint256{0, 0, 0, 0, 0, 0, (uint32_t)(k_offset >> 32), (uint32_t)(k_offset & 0xFFFFFFFF)});
    ...
    if (mode == 3) {
        while (true) {
            uint64_t start_time = milliseconds();
            gpu_contract3_address_work<<<INFERNAL_GRID_SIZE, INFERNAL_BLOCK_SIZE>>>(soul_score_method, origin_address, deployer_address, infernal_key, bytecode);
...
int main(int argc, char *argv[]) {
    ...
    _uint256 infernal_bytecode_hash;
    if (mode == 2 || mode == 3) {
        ...
        infernal_bytecode_hash = cpu_full_keccak(bytecode, (file_size >> 1) - prefix);
        delete[] bytecode;
    }

    Address infernal_origin_address;
    if (mode == 2 || mode == 3) {
    ...
    Address infernal_deployer_address;
    if (mode == 3) {
                ...
                if (m.status == 0) {
                    ...
                    if (m.results_count != 0) {
                        ...
                        for (int i = 0; i < m.results_count; i++) {
                            if (mode == 0) {
                                CurvePoint p = cpu_point_multiply(INFERNAL_POINT, m.results[i]);
                                addresses[i] = cpu_calculate_address(p.x, p.y);
                            } else if (mode == 1) {
                                CurvePoint p = cpu_point_multiply(INFERNAL_POINT, m.results[i]);
                                addresses[i] = cpu_calculate_contract_address(cpu_calculate_address(p.x, p.y));
                            } else if (mode == 2) {
                                addresses[i] = cpu_calculate_contract_address2(infernal_origin_address, m.results[i], infernal_bytecode_hash);
                            } else if (mode == 3) {
                                _uint256 salt = cpu_calculate_create3_salt(infernal_origin_address, m.results[i]);
                                Address proxy = cpu_calculate_contract_address2(infernal_deployer_address, salt, infernal_bytecode_hash);
