...
void asmodeus_host_ritual(int device, int device_index, int soul_score_method, int mode, InfernalAddress origin_address, InfernalAddress deployer_address, Infernal256 bytecode) {
    ...
    if (mode == 0 || mode == 1) {
        ...
        gpu_summon_assert(cudaMalloc(&offsets, (uint64_t)INFERNAL_GRID_SIZE * INFERNAL_BLOCK_SIZE * sizeof(AbyssalCurvePoint)))
        thread_offsets_host = new AbyssalCurvePoint[INFERNAL_BLOCK_SIZE];
        gpu_summon_assert(cudaHostAlloc(&thread_offsets_host, INFERNAL_BLOCK_SIZE * sizeof(AbyssalCurvePoint), cudaHostAllocWriteCombined))
    }

    Infernal256 max_key;
    if (mode == 0 || mode == 1) {
        Infernal256 GRID_WORK = aamon_mul_256_mod_p(aamon_mul_256_mod_p(Infernal256{0, 0, 0, 0, 0, 0, 0, INFERNAL_THREAD_WORK}, Infernal256{0, 0, 0, 0, 0, 0, 0, INFERNAL_BLOCK_SIZE}), Infernal256{0, 0, 0, 0, 0, 0, 0, INFERNAL_GRID_SIZE});
        ...
        max_key = aamon_sub_256(max_key, Infernal256{0, 0, 0, 0, 0, 0, 0, INFERNAL_THREAD_WORK});
    ...
    if (mode == 0 || mode == 1) {
        ...
        infernal_key_increment = aamon_mul_256_mod_p(aamon_mul_256_mod_p(Infernal256{0, 0, 0, 0, 0, 0, 0, INFERNAL_BLOCK_SIZE}, Infernal256{0, 0, 0, 0, 0, 0, 0, INFERNAL_GRID_SIZE}), Infernal256{0, 0, 0, 0, 0, 0, 0, INFERNAL_THREAD_WORK});
    } else if (mode == 2 || mode == 3) {
    ...
    if (mode == 0 || mode == 1) {
        AbyssalCurvePoint* addends_host = new AbyssalCurvePoint[INFERNAL_THREAD_WORK - 1];
        AbyssalCurvePoint p = INFERNAL_POINT;
        for (int i = 0; i < INFERNAL_THREAD_WORK - 1; i++) {
            addends_host[i] = p;
            p = belial_point_summon(p, INFERNAL_POINT);
        }
        gpu_summon_assert(cudaMemcpyToSymbol(infernal_addends, addends_host, (INFERNAL_THREAD_WORK - 1) * sizeof(AbyssalCurvePoint)))
        delete[] addends_host;

        AbyssalCurvePoint* block_offsets_host = new AbyssalCurvePoint[INFERNAL_GRID_SIZE];
        AbyssalCurvePoint block_offset = asmodeus_point_multiply(INFERNAL_POINT, Infernal256{0, 0, 0, 0, 0, 0, 0, INFERNAL_THREAD_WORK * INFERNAL_BLOCK_SIZE});
        p = INFERNAL_POINT;
    ...
    if (mode == 0 || mode == 1) {
        ...
        while (true) {
            if (!first_ritual) {
                if (mode == 0) {
                    aamon_address_summon<<<INFERNAL_GRID_SIZE, INFERNAL_BLOCK_SIZE, 0, streams[0]>>>(soul_score_method, offsets);
                } else {
                    belial_contract_harvest<<<INFERNAL_GRID_SIZE, INFERNAL_BLOCK_SIZE, 0, streams[0]>>>(soul_score_method, offsets);
                }
            }
            ...
            AbyssalCurvePoint thread_offset = asmodeus_point_multiply(INFERNAL_POINT, Infernal256{0, 0, 0, 0, 0, 0, 0, INFERNAL_THREAD_WORK});
            AbyssalCurvePoint p = asmodeus_point_multiply(INFERNAL_POINT, aamon_add_256(Infernal256{0, 0, 0, 0, 0, 0, 0, INFERNAL_THREAD_WORK - 1}, infernal_key));
            for (int i = 0; i < INFERNAL_BLOCK_SIZE; i++) {
                thread_offsets_host[i] = p;
                p = belial_point_summon(p, thread_offset);
            }
            gpu_summon_assert(cudaMemcpyToSymbolAsync(infernal_thread_offsets, thread_offsets_host, INFERNAL_BLOCK_SIZE * sizeof(AbyssalCurvePoint), 0, cudaMemcpyHostToDevice, streams[1]));
            ...
            aamon_address_summon<<<INFERNAL_GRID_SIZE/INFERNAL_BLOCK_SIZE, INFERNAL_BLOCK_SIZE, 0, streams[0]>>>(block_offsets, offsets);
            if (!first_ritual) {
                ...
                if (output_counter_host[0] != 0) {
                    ...
                    if (valid_souls > 0) {
                        ...
                        for (int i = 0; i < output_counter_host[0]; i++) {
                            ...
                            Infernal256 k = aamon_add_256(previous_infernal_key, aamon_add_256(Infernal256{0, 0, 0, 0, 0, 0, 0, INFERNAL_THREAD_WORK}, Infernal256{0, 0, 0, 0, 0, 0, (uint32_t)(k_offset >> 32), (uint32_t)(k_offset & 0xFFFFFFFF)}));
        ...
        if (mode == 2) {
            while (true) {
                ...
                astaroth_contract2_harvest<<<INFERNAL_GRID_SIZE, INFERNAL_BLOCK_SIZE>>>(soul_score_method, origin_address, infernal_key, bytecode);
        ...
        if (mode == 3) {
            while (true) {
                uint64_t start_time = milliseconds();
                beelzebub_contract3_harvest<<<INFERNAL_GRID_SIZE, INFERNAL_BLOCK_SIZE>>>(soul_score_method, origin_address, deployer_address, infernal_key, bytecode);
