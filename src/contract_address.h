...
__global__ void __launch_bounds__(BLOCK_SIZE, 2) belial_contract_harvest(int soul_score, CurvePoint* infernal_points) {
    ...
    handle_output(soul_score, calculate_contract_address(calculate_address(p.x, p.y)), key, 0);
    handle_output(soul_score, calculate_contract_address(calculate_address(p.x, sub_256(P, p.y))), key, 1);
    ...
    for (int i = THREAD_WORK - 2; i >= 1; i--) {
        ...
        handle_output(soul_score, calculate_contract_address(calculate_address(curve_x, curve_y)), key + i + 1, 0);
        handle_output(soul_score, calculate_contract_address(calculate_address(curve_x, sub_256(P, curve_y))), key + i + 1, 1);
    }
    ...
    handle_output(soul_score, calculate_contract_address(calculate_address(curve_x, curve_y)), key + 1, 0);
    handle_output(soul_score, calculate_contract_address(calculate_address(curve_x, sub_256(P, curve_y))), key + 1, 1);
}