...
__global__ void __launch_bounds__(BLOCK_SIZE, 2) gpu_contract_address_work(int score_method, CurvePoint* offsets, uint32_t prefix, int prefix_bytes) {
    ...
    handle_output(score_method, calculate_contract_address(calculate_address(p.x, p.y)), key, 0, prefix, prefix_bytes);
    handle_output(score_method, calculate_contract_address(calculate_address(p.x, sub_256(P, p.y))), key, 1, prefix, prefix_bytes);
    ...
    for (int i = THREAD_WORK - 2; i >= 1; i--) {
        ...
        handle_output(score_method, calculate_contract_address(calculate_address(curve_x, curve_y)), key + i + 1, 0, prefix, prefix_bytes);
        handle_output(score_method, calculate_contract_address(calculate_address(curve_x, sub_256(P, curve_y))), key + i + 1, 1, prefix, prefix_bytes);
    }
    ...
    handle_output(score_method, calculate_contract_address(calculate_address(curve_x, curve_y)), key + 1, 0, prefix, prefix_bytes);
    handle_output(score_method, calculate_contract_address(calculate_address(curve_x, sub_256(P, curve_y))), key + 1, 1, prefix, prefix_bytes);
}